...
public class FileManagementService: ObservableObject {
    ...
    public static func saveObjectToDisc<T:Encodable>(
        ...
        SCMGeneralHelper.Log.info("beginning to save \(filename)")
        ...
        do {
            ...
            SCMGeneralHelper.Log.info("successfully saved \(filename)")
            return fileUrl
        } catch {
    ...
    public static func createSubfolder(subfolder: String){
        if !FileManager.default.fileExists(atPath: subfolder) {
            do{
                ...
                SCMGeneralHelper.Log.info("Failed to create Directory: \(error.localizedDescription)")
            }
        }
...
extension FileManagementService {
  ...
  public static func fileModificationDate(atPath url: URL) -> Date? {
    ...
    SCMGeneralHelper.Log.info("File at \(url.path) last modified at \(date?.description ?? "nil")")
    return date
  }
